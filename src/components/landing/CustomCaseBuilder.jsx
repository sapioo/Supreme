...
export default function CustomCaseBuilder({ onSelectCase }) {
    ...
    return (
        <div className="ccb" id="custom-case-builder">
            {/* Toggle header */}
            <button
                className={`ccb__toggle ${open ? 'ccb__toggle--open' : ''}`}
                onClick={() => setOpen(v => !v)}
                id="custom-case-toggle"
            >
                <span className="ccb__toggle-icon">✦</span>
                <span className="ccb__toggle-text">Build Your Own Case</span>
                <span className="ccb__toggle-sub">Define a custom scenario and argue it live</span>
                <span className="ccb__toggle-chevron">{open ? '▲' : '▼'}</span>
            </button>
